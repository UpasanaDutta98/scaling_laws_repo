...
def save_empirical_netstats(graphname, domain):
    ...
    clustering_coeff = simplified_iG.transitivity_undirected()
    
    degree_assort = simplified_iG.assortativity_degree(directed=False)
    
    distance_list = threshold_sampler_igraph(simplified_iG, threshold=0.1, batch_size=1000)
    avg_path_length = np.mean(distance_list) # storing avg_path_length heuristically here only for completeness. For empirical networks we always go with the true MGD value (computed in store_empirical_trueMGD.py).
    
    Results = [graphname, numnodes, numedges, clustering_coeff, degree_assort, avg_path_length]
    
    pickleFile = "../Results/empirical_netstats/" + domain + "/"+graphname[:-4] + ".pkl"
...
def parallel(list_of_names, domain):
    ...
    for i in range(len(list_of_names)):
        ...
        p = multiprocessing.Process(target=save_empirical_netstats, args=(graphname, domain))
        jobs.append(p)
        p.start()
