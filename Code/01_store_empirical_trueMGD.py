...
def save_empirical_netstats(graphname, domain): # graphname will have ".txt" extension
    ...
    t1 = time.time()
    real_mgd = simplified_iG.average_path_length(directed=False)
    t2 = time.time()
    ...
    sampled_mgd = np.mean(distance_list)
    t2 = time.time()
    
    sampled_time = (t2-t1)/60
    
    Results = [graphname, numnodes, numedges, real_mgd, real_time, sampled_mgd, sampled_time]
    
    pickleFile = "../Results/real_MGD/" + domain + "/"+graphname[:-4] + ".pkl"
