# !pip install ConfigModel_MCMC==0.2 ## (if ConfigModel_MCMC is not already installed).
# Package Repo: https://github.com/UpasanaDutta98/ConfigModel_MCMC
...
def save_Gnm_netstats(graphname, domain):
    G = nx.read_edgelist("../network_repository_edgelists/" + domain + "/"+ graphname)
    ...
    t1 = time.time()
    for i in range(50):
        g_null = mcmc_object.get_graph(return_type = "igraph") # Generate 50 configuration model graphs.
    ...
    pickleFile = "../Results/Config_netstats/" + domain + "/" + graphname[:-4] + ".pkl"
